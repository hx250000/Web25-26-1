...
public class ChatService {
    ...
    public String doChat(long roomId, String userPrompt) {
        ...
        // 关键修改：确保房间存在
        chatHistories.putIfAbsent(roomId, new ArrayList<>());
        List<ChatMessage> messages = chatHistories.get(roomId);

        // 只在房间为空且用户开始游戏时添加系统提示
        if (messages.isEmpty() && userPrompt.equals("开始")) {
        ...
        /* 添加用户消息 */final ChatMessage userMessage = ChatMessage.builder()
        ...
        // 关键修改：游戏结束时不删除房间，只标记状态
        // if(answer.contains("游戏结束")){
        //     chatHistories.remove(roomId);  // 注释掉这一行
        // }
